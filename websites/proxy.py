...
def fetch_product_data(product_url, ua):
    with sync_playwright() as p:
        ...
        try:
            rating_point_element = product_page.query_selector(
                ".product-information__link__reviews"
            )
            rating_point = len(
                rating_point_element.query_selector_all(".icon.icon--star-filled")
            ) + (
                0.5
                if rating_point_element.query_selector(".icon.icon--star-filled-half")
                else 0
            )
        ...
        try:
            rating_count_element = product_page.query_selector(
                ".product-information__link__reviews"
            )
            rating_count = (
                rating_count_element.query_selector_all("span")[-1].text_content()
        ...
        try:
            stock_string = product_page.query_selector(
                ".product-bopis-accordion__content.product-bopis-accordion__content--open > p"
            ).text_content()
        ...
        product_data.product_name = (
            product_page.query_selector("h1.product-title")
            .query_selector(":scope > *")
            .text_content()
            .strip()
            if product_page.query_selector("h1.product-title")
            else None
        )
        product_data.brand = (
            product_page.query_selector("div.product-tile__brand-name")
            .text_content()
            .strip()
            if product_page.query_selector("div.product-tile__brand-name")
            else None
        )
        product_data.category = (
            product_page.query_selector(".breadcrumbs__list").text_content()
            if product_page.query_selector(".breadcrumbs__list")
            else None
        )
        product_data.regular_price = (
            product_page.query_selector(".mini-commerce-summary__price.money.pre-money")
            .text_content()
            .strip()
            if product_page.query_selector(
                ".mini-commerce-summary__price.money.pre-money"
            )
            else None
        )
        product_data.discounted_price = (
            product_page.query_selector("span.product-tile__sale-price")
            .text_content()
            .strip()
            if product_page.query_selector("span.product-tile__sale-price")
            else None
        )
        ...
        product_data.description = (
            product_page.query_selector(".product-details__content").text_content()
            if product_page.query_selector(".product-details__content")
            else None
        )
        ...
        product_data.image_link = [
            ...
            for item in product_page.query_selector_all(
                ".thumbnail-slider__thumb.thumbnail-slider__image"
            )
        ]
        product_data.sku = (
            product_page.query_selector(".product-property")
            .inner_text()
            .split("Model")[0]
            .replace("Item: ", "")
            if product_page.query_selector(".product-property")
            else None
        )
        product_data.upc = None
        product_data.mfr_number = (
            product_page.query_selector(".product-property")
            .inner_text()
            .split("Model:")[1]
            if product_page.query_selector(".product-property")
...
def collect_data(page_url=page_url, max_items=10):
    ...
    with sync_playwright() as p:
        ...
        product_elements = page.query_selector_all(
            ".product-thumbnail__title.product-link"
        )
        product_urls = [
            BASE_URL + product_element.get_attribute("href")
            for product_element in product_elements[:max_items]
        ]
        ...
        with ThreadPoolExecutor(max_workers=3) as executor:
            futures = [
                executor.submit(fetch_product_data, url, ua) for url in product_urls
            ]
